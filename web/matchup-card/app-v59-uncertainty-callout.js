...
const els = {
  ...
  uncertaintyCallout: document.querySelector("#uncertainty-callout"),
...
function renderAuditVisual(audit) {
  ...
  els.auditVisual.innerHTML = `
    <div class="audit-visual-grid">
      ...
      <article class="audit-viz-card audit-sim-card" id="audit-sim-card">
...
function renderNoMatch() {
  ...
  if (els.uncertaintyCallout) { els.uncertaintyCallout.hidden = true; els.uncertaintyCallout.innerHTML = ""; }
...
function renderProbabilities(match) {
  ...
  renderUncertaintyCallout(match);
}

function renderUncertaintyCallout(match) {
  const el = els.uncertaintyCallout;
  if (!el) return;

  const home = match.probabilities?.home ?? 0;
  const draw = match.probabilities?.draw ?? 0;
  const away = match.probabilities?.away ?? 0;
  const isKnockout = match.is_knockout || ["Quarterfinal","Semifinal","Final","Third-place match"].some(
    s => (match.stage || "").includes(s));
  const hasResult = !!match.score
    || match.result !== null && match.result !== undefined
    || /result_available|past_kickoff|modeled_result/.test(match.fixture_status || "");
  const margin = Math.abs(home - away);

  // Only fire pre-match on knockout games with high uncertainty
  const highDraw   = draw > 0.22;
  const tightMargin = margin < 0.08;
  if (!isKnockout || hasResult || (!highDraw && !tightMargin)) {
    el.hidden = true;
    el.innerHTML = "";
    return;
  }

  // KO-adjusted advancement odds: split draw 50/50 for penalties
  const homeAdv = (home + draw * 0.5) * 100;
  const awayAdv = (away + draw * 0.5) * 100;
  const predicted = home >= away ? match.home_team : match.away_team;
  const predicted_adv = home >= away ? homeAdv : awayAdv;
  const other = home >= away ? match.away_team : match.home_team;
  const other_adv = home >= away ? awayAdv : homeAdv;

  // Severity label
  const entropy = -(home*Math.log2(home+1e-9) + draw*Math.log2(draw+1e-9) + away*Math.log2(away+1e-9));
  const maxEntropy = Math.log2(3); // 1.585 bits
  const entropyPct = Math.round(entropy / maxEntropy * 100);

  const penaltyNote = isKnockout
    ? `In a knockout, a draw after 90 mins goes to extra time — and likely a penalty shoot-out. Redistributing draw probability 50/50 gives KO-adjusted advancement odds: <strong>${match.home_team} ${homeAdv.toFixed(0)}%</strong> &middot; <strong>${match.away_team} ${awayAdv.toFixed(0)}%</strong>.`
    : "";

  el.hidden = false;
  el.innerHTML = `
    <div class="unc-label">MODEL UNCERTAINTY NOTE</div>
    <div class="unc-title">${
      margin < 0.04
        ? "The model cannot meaningfully separate these two teams"
        : "High draw probability — prediction confidence is low"
    }</div>
    <div class="unc-body">
      <p>Draw probability (${pct(draw)}) is above the historical KO-stage mean of ~18%.
      A margin of ${(margin * 100).toFixed(1)}&nbsp;pp between the two teams
      ${ margin < 0.08 ? "is within the model\u2019s typical noise floor" : "leaves substantial uncertainty" }.
      The model pick is ${predicted} — but this is not a confident call.</p>
      <p>${penaltyNote}</p>
    </div>
    <div class="unc-meta">Prediction entropy: ${entropy.toFixed(2)}&nbsp;bits / ${maxEntropy.toFixed(2)} max (${entropyPct}% of maximum uncertainty) &middot; Margin: ${(margin*100).toFixed(1)}&nbsp;pp &middot; 90-min outcomes only</div>
  `;
}
